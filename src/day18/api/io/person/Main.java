...
public class Main {
    ...
    private static Person loadData() {
        ...
        try {
            fis = new FileInputStream("D:\\developing_lyj\\iotest\\person.sav");
    ...
    private static void saveData(Person person) {
        ...
        try {
            fos = new FileOutputStream("D:\\developing_lyj\\iotest\\person.sav");

            // 객체를 파일에 저장하려면 보조스트림 ObjectOutputStream을 사용
            ObjectOutputStream oos = new ObjectOutputStream(fos);

            // 객체 세이브 파일에 저장
            // - 객체를 저장하려면 직렬화를 수행해야 함! 직렬화는 Person 클래스로 가서 이어서~
